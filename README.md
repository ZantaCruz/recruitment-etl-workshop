...
### 12.7 Export query results to CSV

`sql/export_results.sql` runs the same five queries and saves each result as a CSV file inside `results/`. Run it from the project root (not from inside `sql/`), since `\copy` paths are resolved relative to your current folder:

```bash
psql -U <user> -d recruitment_dw -f sql/export_results.sql
```

This creates `results/r1_hiring_trend.csv`, `r2_technology_analysis.csv`, `r3_candidate_profile.csv`, `r4_experience_vs_outcome.csv`, and `r5_geographic_analysis.csv`.
...
SELECT
    s.seniority_name,
    CASE
        WHEN f.yoe <= 5  THEN '0-5 years'
        WHEN f.yoe <= 15 THEN '6-15 years'
        ELSE '16+ years'
    END                                                  AS yoe_range,
    COUNT(*)                                            AS total_applications,
    SUM(CASE WHEN f.is_hired THEN 1 ELSE 0 END)         AS total_hired,
...
│   ├── r4_experience_vs_outcome.csv
│   └── r5_geographic_analysis.csv
├── sql/
│   ├── create_tables.sql
